<template>
    <div class="order-item" @click="$emit('click-item')">
      <div class="tit">
        ...
        <div class="status">
          <span :class="{ 'warning': isWarningStatus }">{{ item.state_text }}</span>
        </div>
      </div>
      <div class="list">
      ...
      <div class="actions">
        ...
        <div v-if="item.order_status === 30">
          <span @click.stop="$emit('on-comment', item.order_id)" class="primary">评价</span>
        </div>
      </div>
    </div>
  </template>
...
export default {
  ...
  computed: {
    isWarningStatus () {
      return [0, 10].includes(this.item.order_status)
    }
  }
}
</script>
